...
public:
  ...
  int count_interval(T const& lower, T const& upper) const {
      return count_interval_rec(root_node, lower, upper);
  }
...
private:
  ...
  static int count_interval_rec(Node* node, const T& lower, const T& upper) {
      if (node == nullptr) {
          return 0; // Caso base: rama vacia
      }

      int cont = 0;

      if (node->elem >= lower && node->elem <= upper) { //si es un valor dentro del intervalo,sumamos 1 al contador
          cont = 1;
      }
      //exploramos por ambos lados
      if (node->elem > lower) { // si es mayor que el minimo, sumamos 1 y exploramos por la derecha
          cont += count_interval_rec(node->left, lower, upper);
      }

      if (node->elem < upper) { // si es menor que el maximo, sumamos 1 y exploramos por la izquierda
          cont += count_interval_rec(node->right, lower, upper);
      }
