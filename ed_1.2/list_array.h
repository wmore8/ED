...
void ListArray::reverse_segment(int index, int length) {
    int actual = index, fin = index + length - 1;
    std::string aux = "";

    
    while (fin - actual > 0) { // hasta que no hayamos realizado todos los cambios
        aux = elems[actual];
        elems[actual] = elems[fin];
        elems[fin] = aux;
